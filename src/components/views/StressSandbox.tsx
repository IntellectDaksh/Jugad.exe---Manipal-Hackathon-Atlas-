...
export function StressSandbox() {
  ...
  const comparisonData = useMemo(() => {
    return baselineProj.map((bp, i) => ({
      label: bp.label,
      baselineRSI: bp.projectedRSI,
      stressedRSI: stressedProj[i].projectedRSI
    }));
  }, [baselineProj, stressedProj]);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="card p-5">
          ...
          <div className="mt-4 h-40">
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={baselineProj} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
                ...
                <ReferenceLine y={65} stroke="#ef4444" strokeDasharray="3 3" opacity={0.5} />
                ...
                />
                <Line type="monotone" dataKey="projectedRSI" name="RSI" stroke="#3b82f6" strokeWidth={3} dot={{ r: 4, fill: '#3b82f6', strokeWidth: 0 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="card p-5 border-2 border-warning-200 dark:border-warning-900">
          ...
          <div className="mt-4 h-40">
            <p className="text-[10px] text-ink-500 uppercase tracking-wide mb-2">6-Month RSI Projection</p>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={stressedProj} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#3f3f46" opacity={0.2} />
                <XAxis dataKey="label" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: '#71717a' }} />
                <YAxis domain={[0, 100]} axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: '#71717a' }} />
                <ReferenceLine y={65} stroke="#ef4444" strokeDasharray="3 3" opacity={0.5} />
                ...
                <Tooltip 
                  ...
                  labelStyle={{ color: '#a1a1aa', marginBottom: '4px' }}
                />
                <Line type="monotone" dataKey="projectedRSI" name="RSI" stroke="#f59e0b" strokeWidth={3} dot={{ r: 4, fill: '#f59e0b', strokeWidth: 0 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="card p-5 border border-primary-200 dark:border-primary-900 bg-white dark:bg-ink-900">
        <h3 className="text-sm font-semibold text-ink-900 dark:text-ink-50 mb-4">Scenario Comparison (RSI Overlay)</h3>
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={comparisonData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <Area type="monotone" dataKey="baselineRSI" name="Baseline RSI" stroke="#3b82f6" strokeWidth={2} fillOpacity={1} fill="url(#colorBaseline)" />
              <Area type="monotone" dataKey="stressedRSI" name="Stressed RSI" stroke="#f59e0b" strokeWidth={2} fillOpacity={1} fill="url(#colorStressed)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
